...
X2_test = X2_test.astype('float64').values
y2_test = y2_test.ravel()

# Set up test data as tensor
X_test_tensor = torch.FloatTensor(X2_test)  
y_test_tensor = torch.LongTensor(y2_test) 
# Z matrix are the predictions resulting from the forward pass through the network
test2_pred = model_predict(model, X_test_tensor)

# print(Z)

unique, counts = np.unique(test2_pred, return_counts=True)
print(dict(zip(unique, counts)))

#check scores
score2 = f1_score(y_test_tensor, test2_pred, average='macro')

print(np.round(score2, 3))

# text confusion matrix
print("Confusion Matrix (rows: Predicted class, columns: True class):")
conf_mat = confusion_matrix(y_pred=test2_pred, y_true=y2_test)
print(conf_mat)

# figure confusion matrix
conf_display = ConfusionMatrixDisplay.from_predictions(y_pred=test2_pred, y_true=y2_test, colorbar=False)
